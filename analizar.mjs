...
async function callBlackbox(content, attempt = 0) {
  ...
  const preferredModel = process.env.BLACKBOX_MODEL || '';
  ...
  try {
    ...
    if (attempt < modelPool.length - 1) {
      const delay = [5000, 12000, 25000][attempt];
      console.warn(`⏳ Respuesta no-JSON de Blackbox con ${model} (status ${response.status}) — probando fallback en ${delay / 1000}s...`);
  ...
  if (!response.ok) {
    ...
    if ((is429 || response.status >= 400) && attempt < modelPool.length - 1) {
      const delay = [5000, 12000, 25000][attempt];
      console.warn(`⏳ Blackbox ${response.status} con ${model} — probando fallback en ${delay / 1000}s...`);
      await new Promise(r => setTimeout(r, delay));
      return callBlackbox(content, attempt + 1);
    }
