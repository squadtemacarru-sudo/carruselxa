...
function aplicarSistema(s, wrapper) {
  ...
  if (s.font_display_familia) r.style.setProperty('--font-display', `'${s.font_display_familia}', sans-serif`);
  if (s.font_body_familia)    r.style.setProperty('--font-body',    `'${s.font_body_familia}', sans-serif`);
  if (s.font_mono_familia)    r.style.setProperty('--font-mono',    `'${s.font_mono_familia}', monospace`);
  const p = s.paleta || {};
  ...
  ['font-display-link','font-body-link','font-mono-link'].forEach((id, i) => {
    const url = [s.font_display_url, s.font_body_url, s.font_mono_url][i];
...
function cover(sec, s, layout, useGlass, glassOp) {
  if (layout === 'cover-split') {
    ...
    left.appendChild(el('h1', 'c-headline h-display', esc(s.headline)));
...
function list(sec, s, layout) {
  ...
  if (layout === 'list-full') {
    ...
    items.forEach((item, i) => {
      ...
      li.appendChild(el('span', useSmall ? 'list-text-sm' : 'list-text', esc(item)));
      ul.appendChild(li);
    });
    sec.appendChild(ul);
    return;
  ...
  if (layout === 'list-hero') {
    ...
    items.forEach((item, i) => {
      ...
      li.appendChild(el('span', 'list-text-sm', esc(item)));
      ul.appendChild(li);
    });
    sec.appendChild(ul);
    return;
...
function buildBeforeAfter(slide, i, total) {
  ...
  if (slide.headline || slide.sub) {
    ...
    if (slide.headline) footer.appendChild(el('h2','ba-headline',esc(slide.headline)));
